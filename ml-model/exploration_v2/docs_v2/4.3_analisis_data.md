...
Item dummies adalah representasi one-hot encoding dari 61 item menu. Setiap item diubah menjadi kolom biner: bernilai 1 jika baris data milik item tersebut, 0 jika bukan. Ke-61 kolom ini berfungsi sebagai *intercept* per-item, memungkinkan model global mempelajari level dasar setiap item (misalnya, Kopi Susu Husgendam Ice ~4,6 cangkir/hari, Tubruk ~3,7 cangkir/hari, Menawan ~0,02 cangkir/hari) tanpa harus melatih 61 model terpisah.
...
**Gambar 4.38 Matriks Korelasi Antar Fitur (Top 25), Korelasi tertinggi terjadi antar fitur dalam kelompok yang sama (Roll_Mean vs EWMA, r≈0,9); fitur recency memiliki korelasi rendah dengan fitur lain, mengonfirmasi bahwa recency membawa informasi unik**

![Feature Importance XGBoost](../figures/v2_features/feature_importance_xgb.png)

**Gambar 4.39 Feature Importance XGBoost, `Days_Since_Last_Sale` mendominasi (importance 0,73), diikuti fitur rolling/EWMA; pola DOW ditangkap melalui item dummies dan fitur siklikal**
...
**Gambar 4.40 Hasil Ablasi Kelompok Fitur, Recency adalah kelompok paling kritis (+0,381 MAE jika dihapus); DOW Baselines justru merugikan (-0,053 MAE)**
...
### 4.3.3.4 Set Fitur Final

Berdasarkan seluruh analisis di atas, MI (Sub-bab 4.3.3.2) dan ablasi (Sub-bab 4.3.3.3), ditetapkan set fitur final yang akan digunakan dalam pipeline Bab V. Tabel 4.16 menyajikan ringkasan set fitur final.

**Tabel 4.16 Set Fitur Final (101 Fitur)**

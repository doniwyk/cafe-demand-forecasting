        ...
        <CardContent>
          <div className="flex gap-4 flex-wrap items-end">
                  ...
                  render={
                    <Button variant="outline" className="w-[280px] justify-start text-left font-normal">
                      ...
                      {dateRange.from ? (
                        ...
                        <span className="text-muted-foreground">{t("forecasts.selectDateRange")}</span>
                      )}
                    </Button>
                  }
                />
            ...
            {(selectedItem || dateRange.from || dateRange.to) && (
              <Button variant="ghost" size="sm" onClick={() => { setSelectedItem(null); setDateRange({}) }}>
                {t("common.clear")}
              </Button>
            )}
          </div>
            ...
            forecastForItem.length > 0 ? (
              <div className="flex flex-col gap-4">
                ...
                <Table>
                  ...
                  <TableBody>
                    {(() => {
                      const data = forecasts.data?.data
                        .filter((f) => f.item === selectedItem)
                        .sort((a, b) => a.date.localeCompare(b.date)) ?? []
                      return data.map((f, idx) => (
                        <TableRow key={`${f.date}-${idx}`}>
                          <TableCell className="font-medium">{format(parseISO(f.date), 'MMM dd, yyyy')}</TableCell>
                          <TableCell className="text-right">{Math.round(f.quantity_sold * 10) / 10}</TableCell>
                        </TableRow>
                      ))
                    })()}
                  </TableBody>

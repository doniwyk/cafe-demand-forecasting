...
import { MODEL_TYPES, MODEL_LABELS, type ModelType } from '@/contexts/model-context'
import { PlayIcon, CheckIcon, LoaderIcon, AlertCircleIcon, Trash2Icon, TerminalIcon, ChevronDownIcon, ChevronUpIcon, XIcon } from 'lucide-react'
    ...
    <div className="flex flex-1 flex-col gap-6 p-4">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Train forecasting models individually or all at once (one after another).</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              ...
              <Button
                ...
                disabled={isAnyTraining || retrain.isPending}
              >
                    ...
                    {logs.length > 0 || (st?.status && st.status !== 'idle') && (
                      <div className="mt-2">
                        ...
                        >
                          ...
                          {expandedLogs[mt] ? <ChevronUpIcon className="ml-auto size-3" /> : <ChevronDownIcon className="ml-auto size-3" />}
                        </Button>
                        {expandedLogs[mt] && (
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Remove stale model runs and forecasts from previous training sessions.</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Removes old model runs, their forecasts, class metrics, and top items that are no longer active.
              </p>
              {cleanup.isSuccess && cleanup.data && (
                <p className="text-xs text-muted-foreground mt-1">
                  Deleted {cleanup.data.deleted_runs} runs, {cleanup.data.deleted_forecasts} forecasts
                </p>
              )}
            </div>
            <Button

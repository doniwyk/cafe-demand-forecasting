...
export const api = {
  auth: {
    login(email: string, password: string) {
      return request<{ access_token: string; token_type: string; user: { id: number; email: string; name: string; avatar: string | null } }>(
        "/auth/login",
        { method: "POST", body: JSON.stringify({ email, password }) },
      )
    },
    me() {
      return request<{ id: number; email: string; name: string; avatar: string | null }>("/auth/me")
    },
    getToken,
    setToken,
  },
